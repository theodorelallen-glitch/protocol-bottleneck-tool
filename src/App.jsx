...
const RESULT_GUIDE_MAP = {
  r_pre_foundation_sleep: { guide: "rpb", explanation: "The Research Protocol Bible covers sleep as the first Foundation Gate variable — what inadequate sleep does to GH pulsatility, cortisol patterning, and GLP-1 response specifically, and the practical steps for stabilizing it before any protocol begins." },
  r_pre_foundation_protein: { guide: "rpb", explanation: "The Research Protocol Bible covers protein as a Foundation Gate variable and the specific intake targets relative to lean body mass. It explains why GLP-1-driven appetite suppression so commonly pulls protein down alongside calories and how to correct intake in the context of an active restriction environment." },
  ...
  r_pre_foundation_all: { guide: "rpb", explanation: "The Research Protocol Bible covers the full Foundation Gate in sequence — all three variables in the correct order with the reasoning behind why sequence matters as much as correction." },
  r_pre_gi_concern: { guide: "reta", explanation: "The Retatrutide guide covers GI symptom management in detail — the three distinct presentations, the dose and timing variables that drive each one, and how to distinguish a manageable titration response from a signal that something else needs attention." },
  r_pre_muscle_concern: { guide: "rpb", explanation: "The Research Protocol Bible covers lean mass preservation across all four protocol phases — which inputs protect it, when a GH support compound becomes the rational addition, and the sequencing logic that prevents the most common lean mass mistakes." },
  r_pre_sleep_concern: { guide: "reta", explanation: "The Retatrutide guide covers injection timing in detail — the mechanism behind morning versus evening injection, why timing matters differently for retatrutide than for other GLP-1 compounds, and what the data shows about sleep disruption as a timing variable." },
  r_pre_general_concern: { guide: "rpb", explanation: "The Research Protocol Bible covers the full phase framework from the start — what each phase looks like, what normal variation looks like at each stage, and how to read results accurately rather than reacting to them." },
  r_pre_glp1_mechanism: { guide: "reta", explanation: "The Retatrutide guide covers the full GLP-1 mechanism in plain language — what each receptor does, how that translates into observable results at each phase, and why the same compound works differently for different researchers." },
  r_pre_phase_logic: { guide: "rpb", explanation: "The Research Protocol Bible is built around the phase framework. It covers all four phases in detail — how to identify which one applies, what the rational compound logic is at each stage, and why the sequence matters more than the compound choice itself." },
  r_pre_compound_match: { guide: "rpb", explanation: "The Research Protocol Bible covers the GLP-1 starting point decision and the compound selection framework in the first chapter — how to identify which compound fits the actual limiting variable before starting, not after the first plateau." },
  r_pre_support_logic: { guide: "rpb", explanation: "The Research Protocol Bible covers the support compound decision framework — which bottleneck each support compound addresses, when that bottleneck becomes the active limiting variable, and why the sequencing matters more than the compound itself." },
  ...
  r_pre_ready: { guide: "rpb", explanation: "The Research Protocol Bible covers the starting point decision and the compound selection framework in detail — how to confirm the match between your limiting variable and the compound mechanism before committing to a protocol." },
  ...
  r_phase1_early: { guide: "reta", explanation: "The Retatrutide guide covers the titration window in detail — what the research shows about weeks one through six, how to read early signals correctly, and why escalating the dose before the effective range is established almost always produces a worse outcome than waiting." },
  r_no_intake: { guide: "reta", explanation: "The Retatrutide guide covers the intake suppression mechanism and what an absent appetite response actually signals. It walks through the three most common causes — under-dosing, active titration, and reconstitution error — and how to distinguish between them." },
  r_caloric_audit: { guide: "reta", explanation: "The Retatrutide guide covers the Phase 2 metabolic adaptation pattern directly — why a real deficit at week 4 often becomes a marginal one by week 12, and how to run the 7-day intake audit that identifies the actual gap." },
  r_foundation_sleep: { guide: "rpb", explanation: "The Research Protocol Bible covers the full Foundation Gate framework — the four inputs that determine whether any compound can produce a measurable result. Sleep is Gate 1, and the RPB covers the specific GLP-1 timing and dose variables that most commonly drive disruption." },
  r_foundation_stimulant: { guide: "rpb", explanation: "The Research Protocol Bible covers the Foundation Gate including the stimulant load variable and why daily use creates a cortisol ceiling that caps what any metabolic compound can accomplish." },
  r_foundation_protein: { guide: "rpb", explanation: "The Research Protocol Bible covers protein as a Foundation Gate variable and why GLP-1-driven appetite suppression so commonly pulls protein intake down alongside calories." },
  ...
  r_bn02_early: { guide: "reta", explanation: "The Retatrutide guide covers the three checks to run before interpreting a recent stall as a bottleneck — precise caloric tracking, dose and timing review, and compound-specific stall pattern identification." },
  r_bn02: { guide: "reta", explanation: "The Retatrutide guide covers the Phase 2 fat loss pattern in detail — what drives the transition from intake-driven to output-driven fat loss, which fat mobilization variables the research points to at this stage, and how to evaluate whether your protocol has genuinely entered Phase 2." },
  ...
  r_bn03_dose: { guide: "reta", explanation: "The Retatrutide guide covers the dose-driven energy drop pattern and how to distinguish it from a genuine energy bottleneck. When energy falls shortly after an escalation, the compound is most likely creating a deficit the body cannot sustain output under." },
  r_bn03: { guide: "rpb", explanation: "The Research Protocol Bible covers all three presentations of the gradual energy decline pattern and the compound logic that applies to each. Getting the presentation right matters more than the compound choice itself." },
  ...
  r_bn04: { guide: "rpb", explanation: "The Research Protocol Bible covers the sleep and recovery bottleneck protocol — the timing and dose variables to check first, and the specific compounds the research supports for each presentation of sleep disruption during an active protocol." },
  r_bn05: { guide: "reta", explanation: "The Retatrutide guide covers GI symptoms in the context of an active GLP-1 protocol and how to distinguish between the three presentations — dose timing driven nausea, gut motility issues from sustained suppression, and mucosal stress from extended restriction." },
  r_bn06: { guide: "rpb", explanation: "The Research Protocol Bible covers the mood and motivation bottleneck and the diagnostic sequence for identifying whether the upstream driver is cortisol elevation, hormonal suppression, or energy depletion." },
  r_bn07: { guide: "rpb", explanation: "The Research Protocol Bible covers the dose escalation pattern and the five checks to run before any further dose change. The RPB is direct on this: when escalating the dose does not move the plateau, the dose was not the limiting variable." },
  r_bn07_stall: { guide: "rpb", explanation: "The Research Protocol Bible covers the pre-escalation checklist and the Phase 2 assessment that almost always identifies the actual limiting variable when a stall triggered a dose increase." },
  r_phase3: { guide: "stack", explanation: "The Retatrutide, Tesamorelin, and Ipamorelin guide covers Phase 3 recovery strain and the compound logic that applies when training performance declines alongside a stall. This is a different compound framework than Phase 2." },
  r_phase4: { guide: "rpb", explanation: "The Research Protocol Bible covers the Phase 4 framework and what the research supports for rebuilding recovery capacity when the sleep-stall-recovery pattern has collapsed together." },
  r_gh_insulin: { guide: "stack", explanation: "The Retatrutide, Tesamorelin, and Ipamorelin guide covers the fasted window requirement for GH secretagogues in detail — including how an active GLP-1 protocol changes the required window by slowing gastric emptying." },
  r_gh_bedtime: { guide: "stack", explanation: "The Retatrutide, Tesamorelin, and Ipamorelin guide covers injection timing for every GH secretagogue in the stack and explains specifically why bedtime Tesamorelin is the most common timing mistake." },
  r_gh_mixed: { guide: "stack", explanation: "The Retatrutide, Tesamorelin, and Ipamorelin guide covers the consistency requirements for GH secretagogue protocols and why varying the injection window produces inconsistent results regardless of compound quality." },
  r_gh_environment: { guide: "stack", explanation: "The Retatrutide, Tesamorelin, and Ipamorelin guide covers the environmental inputs that govern GH pulse quality — sleep depth, cortisol context, insulin environment, and somatostatin brake at injection time." },
  r_repair_none: { guide: "rpb", explanation: "The Research Protocol Bible covers the tissue repair bottleneck including the three most common causes of a null result — reconstitution error, dose inconsistency, and environmental suppression of the repair signal." },
  r_repair_slow: { guide: "rpb", explanation: "The Research Protocol Bible covers the tissue repair framework and how to evaluate whether the compound combination is correctly matched to the repair presentation." },
  r_longevity_general: { guide: "motsc", explanation: "The MOTS-C and SS-31 guide covers the general longevity stack decision directly — when each compound becomes the rational choice, how they interact with each other, and why foundational inputs need to be reasonably stable before longevity compounds produce their most meaningful results." },
  r_immune: { guide: "motsc", explanation: "The MOTS-C and SS-31 guide covers immune resilience and cellular support compounds and how to distinguish between the patterns that each one addresses." },
  r_semax: { guide: "rpb", explanation: "The Research Protocol Bible covers the cognitive and mood section including Semax, Selank, and DSIP — when each one is the rational choice and how they interact with an active fat loss protocol." },
  r_selank: { guide: "rpb", explanation: "The Research Protocol Bible covers the full cognitive and mood framework including when Selank is the correct choice over Semax based on the symptom presentation." },
  r_cog_compound: { guide: "rpb", explanation: "The Research Protocol Bible covers compound interaction logic and how to systematically isolate which variable is driving a cognitive or mood change when the timing points to a recent protocol change." },
};
...
function CTABlock({ resultKey }) {
  ...
  return (
    <div className="pt-cta-block">
      <div className="pt-cta-label">The right research guide for this result</div>
